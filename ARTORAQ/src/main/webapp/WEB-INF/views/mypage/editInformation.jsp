...
$(document).ready(function() {
   ...
   //      history.go(-1) //뒤로가기
    ...
    //휴대폰 번호 체크 자동 하이픈 추가 
    $("#memphoneck").keydown(function(event) {   //입력창에 숫자 입력
        var key = event.charCode || event.keyCode || 0;
        $text = $(this); 
    ...
    $('#save').click(function (e){
    	...
    	// let reg_pw2 = /(?=.*[a-zA-ZS])(?=.*?[#?!@$%^&*-]).{8,16}/; // 문자와 특수문자 조합의 6~24 자리
    	...
    	// 모두 정상 값을 입력했을 경우
    	if(idSuccess && pwSucc && phoneSuccess && nickname .......){
    		// 폼 엘리먼트.submit();
    		$('#save').submit();
    	}
    })
			...
			<div class="form">
				<div class="form-group">
					<div class="button-wrap">
						<!-- html에서 form 형식으로 컨트롤러에게 데이터 전송시, 
						컨트롤러는 html의  form 엘리먼트 객체 내의 input name값과 value값을 전달받게 됨 
						name값은 key로 value값은 value로 들어가게되어
						
						값을 전송하기 위해서는 컨트롤러의 파라미터로 지정된 객체 내 변수명과 일치해야한다
						
						컨트롤러가 memid 값을 받아주지 못하고 있었기 때문에
						<input type="hidden" type="memid" value="{세션값}"> 으로 
						컨트롤러에게 memid를 전송해주었음.
							-->
					...
					<div class="button-wrap">
						...
						<div class="col-xs-10">
							<input type="text" class="form-control" id="memphone"
								name="memphone" maxlength="13" required>
