...
//						}
					...
					if( !item.isFormField() ) {
						...
						System.out.println("원본파일명 " + item.getName() );
	...
	public List<UploadFile> getFile() {
		return boardDao.getFile(JDBCTemplate.getConnection());
	}
	...
	public void update(HttpServletRequest req) {
		...
		//multipart/form-data 인코딩 확인
			...
			if( !isMultipart ) {
				...
				/*
				 * //게시글 번호 생성 String boardno = boardDao.selectNextBoardno(conn);
				 */
				...
				if ( uploadFile.getUploadname() != "") {
					...
					System.out.println(uploadFile.getBoardno());
					if ( boardDao.insertFile(conn, uploadFile) > 0) {
						JDBCTemplate.commit(conn);
					} else {
						JDBCTemplate.rollback(conn);
