...
import dao.impl.BoardReviewDaoImpl;
import dto.Board;
import dto.Member;
import dto.Review;
...
public class BoardReviewServiceImpl implements BoardReviewService {
	...
	private BoardReviewDao boardReviewDao = new BoardReviewDaoImpl();
	private BoardReviewDao commentDao = new BoardReviewDaoImpl();
...
//		board.setAdminId((String) req.getSession().getAttribute("adminId")) ;
		...
		if(boardReviewDao.insert(conn, board) > 0) {
...
//		} 
	...
	public List<Board> search(String result) {
		...
		return boardReviewDao.searchByKeyword(conn, result);
	}

	@Override
	public int cart( HttpServletRequest req ,String boardno) {
		Board board = new Board();  //보드객체 생성
		
	//	System.out.println("boardno = " + boardReviewDao.selectBoadrnoByTitle( conn, title));
		board.setBoardno( boardno ); //게시물 번호 가져오기
		...
		int res = boardReviewDao.check(conn, board , mem); // 위시리스트에서 회원번호와 회원아이디 일치하는 결과물 유무 확인
		
		if( res != 1) { //카트에 없으면 넣고, 있으면 삭제
			return boardReviewDao.cartAdd( conn, mem, board );    // 위시리스트에 입력  
		} else {
			return boardReviewDao.cartOut( conn, mem, board ); 	// 위시리스트에 삭제
		}
